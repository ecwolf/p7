...
def generate_p4(rec_port, port_user, name_sw, hosts, links,
	...
	if (len(hosts) > 0):
		...
		f.write("const bit<16> maxPercent = 0x3fc;	// 100 percent of Loss value\n")
	else:
	...
	f.write("    bit<16>  R;\n")
	f.write("    bit<16>  R_send;\n")
	f.write("    bit<16>  prob_r;\n")
	f.write("    bit<16>  prob_p;\n")
	if (routing_model == 1):
		f.write("\n")
		f.write("    // PolKa\n")
		if (crc == 8):
	...
	f.write("\n")
	f.write("    // Register to validate the Jitter value\n")
	f.write("    Register <bit<32>, bit<16>> (32w1)  ax;\n")
	f.write("\n")
	f.write("    // Register to valdiate the packet loss model\n")
	f.write("    Register <bit<16>, bit<16>> (32w1024)  pkt_losscal;\n")
	f.write("	 Register <bit<16>, bit<16>> (32w1024)  transition_state_p;\n")
	f.write("	 Register <bit<16>, bit<16>> (32w1024)  transition_state_r;\n")
	f.write("	 Register <bit<16>, bit<16>> (32w1024)  probability_send_k;\n")
	f.write("	 Register <bit<16>, bit<16>> (32w1024)  probability_send_h;\n")
	f.write("	 Register <bit<16>, bit<16>> (32w1024)  state;\n")
	f.write("	 Register <bit<16>, bit<16>> (32w1024)  pkt_loss_model;\n")
	f.write("\n")
	...
	f.write("	RegisterAction<bit<16>, bit<16>, bit<16>>(pkt_loss_model) pkt_loss_model_action = {\n")
	f.write("        void apply(inout bit<16> model, out bit<16> readvalue){\n")
	f.write("            readvalue = model;\n")
	f.write("        }\n")
	f.write("    };\n")
	f.write("\n")
	...
	# Register for jitter
	f.write("    RegisterAction<bit<32>, bit<16>, bit<8>>(ax) ax_action = {\n")
	...
	# Registers for Gilbert-Elliott packet loss model
	f.write("    RegisterAction<bit<16>, bit<16>, bit<16>>(transition_state_p) transition_state_p_action = {\n")
	f.write("        void apply(inout bit<16> p, out bit<16> readvalue){\n")
	f.write("            if ( p > md.R ) {  // if p > R, go to bad state\n")
	f.write("                readvalue = 0;\n")
	f.write("            }else {         // else p < R, keep to good state\n")
	f.write("                readvalue = 1;\n")
	f.write("            }\n")
	f.write("        }\n")
	f.write("    };\n")
	f.write("\n")
	f.write("    RegisterAction<bit<16>, bit<16>, bit<16>>(transition_state_r) transition_state_r_action = {\n")
	f.write("        void apply(inout bit<16> r, out bit<16> readvalue){\n")
	f.write("            if ( r > md.R ) {  // if r > R, go to good state\n")
	f.write("                readvalue = 1;\n")
	f.write("            }else {         // else r < R, keep to bad stae\n")
	f.write("                readvalue = 0;\n")
	f.write("            }\n")
	f.write("        }\n")
	f.write("    };\n")
	f.write("\n")
	f.write("    RegisterAction<bit<16>, bit<16>, bit<16>>(probability_send_k) probability_send_k_action = {\n")
	f.write("        void apply(inout bit<16> k, out bit<16> readvalue){\n")
	f.write("            if ( md.R_send < maxPercent - k ) {\n")
	f.write("                readvalue = 0;\n")
	f.write("            } else {\n")
	f.write("                readvalue = 1;\n")
	f.write("            }\n")
	...
	f.write("    };\n")
	f.write("\n")
	f.write("    RegisterAction<bit<16>, bit<16>, bit<16>>(probability_send_h) probability_send_h_action = {\n")
	f.write("        void apply(inout bit<16> h, out bit<16> readvalue){\n")
	f.write("            if ( h > md.R_send ) {\n")
	f.write("                readvalue = 1;\n")
	f.write("            } else {\n")
	f.write("                readvalue = 0;\n")
	f.write("            }\n")
	...
	f.write("    };\n")
	f.write("\n")
	f.write("    RegisterAction<bit<16>, bit<16>, bit<16>>(state) state_action = {\n")
	f.write("        void apply(inout bit<16> state_value, out bit<16> readvalue){\n")
	f.write("			if (state_value == 1){   // GOOD_CASE - apply p condition\n")
	f.write("				state_value = md.prob_p;\n")
	f.write("			}else {                    // BAD_CASE - apply r condition\n")
	f.write("				state_value = md.prob_r;\n")
	f.write("			}\n")
	f.write("			readvalue = state_value;\n")
	f.write("        }\n")
	f.write("    };\n")
	f.write("\n")
	...
	if (routing_model == 2):
		f.write("        ig_intr_tm_md.ucast_egress_port = ig_intr_md.ingress_port;\n")
	else:
	...
	f.write("                        // Thanks Leonardo Marques for the packet loss model contribution\n")
	f.write("                        // https://github.com/l-io\n")	
	f.write("                        if (pkt_loss_model_action.execute(select_sw) == 1){\n")
	f.write("                            md.prob_p = transition_state_p_action.execute(select_sw);\n")
	f.write("                            md.prob_r = transition_state_r_action.execute(select_sw);\n")
	f.write("                            md.R_send = (bit<16>)rnd.get();\n")
	f.write("                            // change state according md.prob_p or md.prob_r\n")
	f.write("                            if ( state_action.execute(select_sw) == 1) {\n")
	f.write("                                if ( probability_send_k_action.execute(select_sw) == 0 ) {\n")
	f.write("                                   drop();\n")
	f.write("                                } else {\n")
	if (routing_model == 1):
	...
	f.write("                                }\n")
	f.write("                            } else {\n")
	f.write("                                if ( probability_send_h_action.execute(select_sw) == 1 ) {\n")
	f.write("                                    basic_fwd.apply();\n")
	if (routing_model == 1):
		f.write("                                    basic_fwd_hash.apply();\n")
	f.write("                                } else {\n")
